...
- 駅すぱあと API MCPサーバーを VS Code から利用できるようにする
...
| プロンプト例 | 「Googleで駅すぱあとを検索してスクリーンショットを撮って」 |
...
#### ハンズオン: 駅すぱあと公式サイトを開いてスクリーンショットを撮る
```txt
...
2. ブラウザが開き、駅すぱあとの公式サイトが開いたことを確認する
...
- 駅すぱあと APIのアクセスキーを用意している
- ターミナルで環境変数を設定できる

**Step 1: 環境変数を設定する**

駅すぱあと APIのアクセスキーを環境変数に登録します。
...
- 駅すぱあと API MCPサーバーで経路検索や駅情報取得を自然言語で実行できる
